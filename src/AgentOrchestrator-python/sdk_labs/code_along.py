...
Two kinds of marker, and the difference matters on camera:
    ...
    # TYPE THIS LIVE   you genuinely type this. Phase 3 only. One line.
...
# The description strings are not comments. They are the contract: that plain
# English is how the model knows what this function does and when to reach for
# it. Read them out loud. Do not retype the block on camera.
...
async def phase3_tools(requested_model_id: str | None) -> int:
    ...
    async with CopilotClient() as client:
        ...
        #
        # Say: "I never wrote a line of that. It went and read my filesystem.
        #       That's the default posture — capable, and permissive. Now watch
        #       what happens when I hand it one function I control."
        #
        # Then add the tool below and run it again.

        # TYPE THIS LIVE — the only thing you type all session ───────────────
        # Put get_customer_total inside the empty tools list below, so it reads:
        #
        #     tools=[get_customer_total],
        #
        # One line, on purpose: maximum drama, zero typo risk on camera.
        #
        # NOTE — Python makes you pass on_permission_request. This one is
        # approve_all: it approves silently, so do NOT present it on stage as a
        # gate the audience watched work. Two more things to be precise about:
        #   * tools=[...] ADDS your tool. It does not remove the CLI's built-in
        #     file and shell tools — that's why the "before" run could rummage.
        #   * the restriction control is the available_tools allowlist, and it
        #     filters everything, so your own tool must be named in it too.
        #     Not demoed here. Verify it before you rely on it.
...
async def phase5_mcp(requested_model_id: str | None) -> int:
    ...
    async with CopilotClient() as client:
        ...
        # WALK THROUGH — already written. The mcp_servers block is the payload
