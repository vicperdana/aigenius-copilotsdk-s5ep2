"""AI Genius S5E2 — LIVE CODE-ALONG (FINAL / fallback).

Run one phase at a time::

    uv run python -m sdk_labs codealong-final --phase 3

This is the PARACHUTE. Every phase is complete and verified working.

    # FINAL — DO NOT EDIT

Nothing in here is typed live. If the starter breaks on stage, run this instead.
It's a separate command, so a half-typed starter can never stop it running.
"""
...
# FINAL — DO NOT EDIT — the params model + the decorator ──────────────────────────
...
async def phase3_tools(requested_model_id: str | None) -> int:
    ...
    async with CopilotClient() as client:
        ...
        # FINAL — DO NOT EDIT ─────────────────────────────────────────────────────
        # Change tools=[] to tools=[get_customer_total].
        #
...
async def phase4_persistence(requested_model_id: str | None) -> int:
    ...
    async with CopilotClient() as client:
        ...
        # FINAL — DO NOT EDIT — the id is the whole trick ─────────────────────────
...
async def phase5_mcp(requested_model_id: str | None) -> int:
    ...
    async with CopilotClient() as client:
        ...
        # FINAL — DO NOT EDIT — the mcp_servers block is the payload ──────────────
...
async def run(phase: int, requested_model_id: str | None) -> int:
    ...
    if handler is None:
        print(
            "Usage:\n"
            "  uv run python -m sdk_labs codealong-final --phase <1-5>\n\n"
